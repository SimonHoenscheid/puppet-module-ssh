...
class ssh (
  ...
  $sshd_config_kbdinteractiveauthentication = undef,
  ...
  if $sshd_kerberos_or_local_passwd != undef {
    validate_re($sshd_kerberos_or_local_passwd, '^(yes|no)$', "ssh::sshd_kerberos_or_local_passwd may be either 'yes' or 'no' and is set to <${sshd_kerberos_or_local_passwd}>.")
  }
  if $sshd_kerberos_ticket_cleanup != undef {
    validate_re($sshd_kerberos_ticket_cleanup, '^(yes|no)$', "ssh::sshd_kerberos_ticket_cleanup may be either 'yes' or 'no' and is set to <${sshd_kerberos_ticket_cleanup}>.")
  }
  ...
  if $sshd_x11_display_offset != undef {
    if is_integer($sshd_x11_display_offset) == false { fail("ssh::sshd_x11_display_offset must be an integer and is set to <${sshd_x11_display_offset}>.") }
  }
  ...
  if $sshd_config_kbdinteractiveauthentication != undef {
    validate_re($sshd_config_kbdinteractiveauthentication, '^(yes|no)$', "ssh::sshd_config_kbdinteractiveauthentication may be either 'yes' or 'no' and is set to <${sshd_config_kbdinteractiveauthentication}>.")
  }
  ...
  if $sshd_gssapienablek5users != undef {
    validate_re($sshd_gssapienablek5users, '^(yes|no)$', "ssh::sshd_gssapienablek5users may be either 'yes' or 'no' and is set to <${sshd_gssapienablek5users}>.")
  }
  if $sshd_gssapistrictacceptorcheck != undef {
    validate_re($sshd_gssapistrictacceptorcheck, '^(yes|no)$', "ssh::sshd_gssapistrictacceptorcheck may be either 'yes' or 'no' and is set to <${sshd_gssapistrictacceptorcheck}>.")
  }
  if $sshd_gssapistorecredentialsonrekey != undef {
    validate_re($sshd_gssapistorecredentialsonrekey, '^(yes|no)$', "ssh::sshd_gssapistorecredentialsonrekey may be either 'yes' or 'no' and is set to <${sshd_gssapistorecredentialsonrekey}>.")
  }
  if $sshd_gssapikexalgorithms != undef {
    validate_string($sshd_gssapikexalgorithms)
  }
